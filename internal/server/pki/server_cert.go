...
func GenerateDynamicServerCertificate(caCert *x509.Certificate, caPrivateKey *ecdsa.PrivateKey, host string) (*tls.Certificate, error) {
	...
	if err != nil {
		slog.Error("Failed to generate server P-256 cryptographic key pair", "error", err)
	...
	if err != nil {
		slog.Error("CSPRNG entropy extraction failed for server cert serial number generation", "error", err)
	...
	slog.Debug("Signing dynamic server TLS certificate via Server CA root key", "host", host)
	serverCertDER, err := x509.CreateCertificate(rand.Reader, &template, caCert, &serverPrivKey.PublicKey, caPrivateKey)
	if err != nil {
		slog.Error("PKI factory failed to sign dynamic server x509 certificate", "error", err)
	...
	slog.Info("Successfully generated dynamic server TLS certificate context", "host", host)
