...
Для обхода ограничений инкапсуляции приватных полей и сохранения иммутабельности бизнес-логики, пакет разделен на строгую доменную модель `Config` (доступную только через геттеры) и плоские структуры DTO для сериализации в YAML. Вся разметка полностью совместима с рендером VSCode.
...
Процесс сканирования окружения, разбора дефолтных путей спецификации XDG и сборки объектного графа. Текст сообщений экранирован кавычками для корректного отображения в VSCode.
